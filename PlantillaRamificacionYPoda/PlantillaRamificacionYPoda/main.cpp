...
/*
    1. Formatos
        a. SoluciÛn: Tupla (p1,..,pm) para el i-esimo comensal se le asigna la plaza pi.
        b. Restricciones explÌcitas: para todo pi: 1<=pi<=n: es una plaza
        c. Resticciones implÌcitas: Se maximiza los allegados, cada comensal se le debe asignar una plaza.
            Los no allegados se deben sentar a una distancia de al menos 2 metros.
        d. EstimaciÛn: Como no es siempre posible sentar a todos, no se puede hacer cota pesimista.
            La estimaciÛn ser· que todos los que quedan son allegados y son allegados a los que ya est·n sentados.
            La estimaciÛn mejor es el n˙mero de allegados que hay para la persona, guardado en un vector, recorriendo
            solo una diagonal de la matriz de allegados.
        e. ¡rbol:
            |                x
            |               / \
            |              /   \
...
struct Nodo {
    ...
    bool operator < (Nodo const& otro) const {
        return otro.calidad_estimada > calidad_estimada; //para maximizacion
    }
};
...
void quitanieves_rp(const vector<vector<int>> &calidades, const vector<int> &anchura_carro, const vector<int> &anchura_carretera, const vector<int> &max_acum, vector<int> &mejor_sol, int &mejor_calidad, int n, int m) {
    ...
    cola.push(Y);
    mejor_calidad = calculo_pesimista;
